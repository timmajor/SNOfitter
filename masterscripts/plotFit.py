...
fitFile=""
for rootfile in glob.glob("*cones.root"):
    fitFile=rootfile

#fitFile = "PTXI_______<NUMBEROFSTEPS>steps_1cones.root"
...
#        print correctedTime
        if True: #np.abs(correctedTime)<5:
...
def getFitData(infile,fast=True):
    ...
    if (not fast):
        for step in range(bayesTree.steps):
            ...
            leslieIntensity.append(bayesTree.leslieIntensity[step])
            unknownIntensity.append(bayesTree.unknownIntensity[step])
            if (nCones>0):
...
#              coneIntensity[cone].append(bayesTree.coneIntensity[step*nCones + cone])
    ...
    outputTree={
      ...
      'leslieIntensity' : leslieIntensity,
      'unknownIntensity' : unknownIntensity,
...
if (plotType==1):
    ...
    xArray, yArray, zArray, phiArray, thetaArray, nPMTs = getPMTData("/home/tim/sno_code/fittingcode/"+pmtdatafile,fitData)
#    grid2D.SetMarkerStyle(20)
    ...
    if (fitOn>0):
        ...
        for cone in range(fitData["nCones"]):
            ...
            cone2D[cone].SetMarkerColor(ROOT.kGreen+int(fitData["mconeIntensity"][cone]*50))
            ...
            ring2D[cone].SetMarkerColor(ROOT.kGreen+int(fitData["mconeIntensity"][cone]*50))
...
#    print nPMTs
    ...
    if (fitOn):
        ...
        for cone in range(fitData["nCones"]):
            ...
            ring1D[cone].SetMarkerColor(ROOT.kGreen+int(fitData["mconeIntensity"][cone]*50))
...
if (plotType == 2):
    ...
    for i in range(len(fitData["lnP"])):
        ...
        if (float('Inf')==fitData["lnP"][i]):
            print i, "-inf"
        if (fitData["lnP"][i] < 100000000 and fitData["lnP"][i]>-1e100):
            oknum += 1
        else:
    ...
    saveConeIndex=False
    if plotValue == "all":
        plotValueArray = ["r_x", "r_y", "r_z","alpha","flatIntensity","unknownIntensity", "coneAngle", "lightSpeed","trms","t"]
#       plotValueArray = plotValueArray + ["u_x","u_y","u_z","coneIntensity"]
        ...
        plotValueArray = plotValueArray + ["lnP"]
        plotValueArray = plotValueArray + ["leslieIntensity"]
