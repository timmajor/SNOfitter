...
#        c0.Update()
#        c0.SaveAs(title+".png")
    cone_flat[-1] = ROOT.TGraph(len(variableDict["coneness"]["data"]),array("d",variableDict["coneness"]["data"]),array("d",variableDict["leslieness"]["data"]))
    ...
    cone_flat[-1].GetXaxis().SetLimits(0.0,1.0)
    cone_flat[-1].GetXaxis().SetTitle("cone intensity")
    cone_flat[-1].GetYaxis().SetRangeUser(0.0,1.0)
    cone_flat[-1].GetYaxis().SetTitle("leslie intensity")
