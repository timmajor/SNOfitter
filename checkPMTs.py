...
def getPMTData(infile, event):
    ...
    for i in range(nPMTs):
        ...
        n.append(tPMT.pmtN[i])
    xArray = array("d",x)
    yArray = array("d",y)
    zArray = array("d",z)
    tArray = array("d",t)
    nArray = array("i",n)
    return xArray, yArray, zArray, tArray, nArray, nPMTs 
...
def getratdata():
    ratfile=open("/home/tim/snoing/install/rat-dev/data/pmt/snoman.ratdb","r")
    ratstuff=ratfile.readlines()
    x=ratstuff[37][5:-6]
    x1=x.split(",")
    x2=[]
    for element in x1:
        try:
            x3=float(element.strip(" "))/10.0
            x2.append(x3)
        except:
            pass
    y=ratstuff[39][5:-6]
    ...
    for element in z1:
        try:
            z3=float(element.strip(" "))/10.0
            z2.append(z3)
        except:
            pass

#PMT types are as follows:
#           1 = normal; 2 = OWL; 3 = Low Gain; 4 = BUTT; 5 = Neck
#           6 = Calib channel;  10 = spare;  11 = invalid
    type0=ratstuff[51][8:-6]
    type1=type0.split(",")
    type2=[]
    for element in type1:
        try:
            type3=int(element.strip(" "))
            type2.append(type3)
        except:
            pass
    return x2, y2, z2, type2


ROOT.gROOT.Reset()

x, y, z, type0 = getratdata()
#print type0[:10]
#xfile=open("pmtdata/x.pmt","w")
#yfile=open("pmtdata/y.pmt","w")
#zfile=open("pmtdata/z.pmt","w")
#type0file=open("pmtdata/n.pmt","w")
#array("d",x).tofile(xfile)
#array("d",y).tofile(yfile)
#array("d",z).tofile(zfile)
#array("i",type0).tofile(type0file)
#xfile.close()
#yfile.close()
#zfile.close()
#type0file.close()
print len(x), len(y), len(z), len(type0)
for event in range(0):
    xArray, yArray, zArray, tArray, nArray, nPMTs = getPMTData("8B_readable_pmtN.root",event)
    for i in range(nPMTs):
#        if type0[nArray[i]] is not 1:
            diffx = xArray[i]-x[nArray[i]]
            diffy = yArray[i]-y[nArray[i]]
            diffz = zArray[i]-z[nArray[i]]
            diff = np.sqrt(diffx*diffx+diffy*diffy+diffz*diffz)
            if diff >1:
                 print diff
#            print type0[nArray[i]], diff
